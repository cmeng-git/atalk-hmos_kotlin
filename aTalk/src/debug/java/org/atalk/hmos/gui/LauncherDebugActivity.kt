 ...
 */
class LauncherDebugActivity : OSGiActivity() {
    /**
    ...
    override fun start(bundleContext: BundleContext?) {
        ...
        runOnUiThread {
            if (restoreIntent != null) {
                ...
                val dailyCheckupIntent = Intent(applicationContext, OnlineUpdateService::class.java)
                dailyCheckupIntent.action = OnlineUpdateService.ACTION_AUTO_UPDATE_START
                startService(dailyCheckupIntent)
            }

            // Start home screen Activity
            val activityClass: Class<*> = aTalkApp.homeScreenActivityClass
