 ...
 */
interface MasterPasswordInputService {
    /**
     * Shows an input dialog to the user to obtain the master password.
     *
     * @param prevSuccess `true` if any previous call returned a correct
     * master password and there is no need to show an extra "verification
     * failed" message
     * @return the master password obtained from the user or `null` if
     * none was provided
     */
    fun showInputDialog(prevSuccess: Boolean): String?
}