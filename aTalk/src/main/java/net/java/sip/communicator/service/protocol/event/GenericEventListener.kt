...
interface GenericEventListener {
     ...
     * the incoming event.
     */
    fun notificationReceived(event: GenericEvent?)

    /**
     * Indicates that a `GenericEvent` we previously tried to send has not been delivered.
     *
     * @param event
     * the `GenericEvent` instance describing the event that we couldn't send.
     */
    fun notificationDeliveryFailed(event: GenericEvent?)
}