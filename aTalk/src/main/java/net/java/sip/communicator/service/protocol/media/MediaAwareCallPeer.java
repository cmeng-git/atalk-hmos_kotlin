...
public abstract class MediaAwareCallPeer<
    ...
    public MediaAwareCallPeer(T owningCall) {
        ...
        mPPS = owningCall.getSourceProvider();
     ...
     */
    public void addStreamSoundLevelListener(@NonNull SoundLevelListener listener) {
     ...
     *
     * @param conferenceEvent the event containing information (that we don't really use) on the freshly removed member.
     */
    ...
    public void removeConferenceMembersSoundLevelListener(
            @NonNull ConferenceMembersSoundLevelListener listener) {
        synchronized (conferenceMembersSoundLevelListeners) {
     ...
     */
    public void removeStreamSoundLevelListener(@NonNull SoundLevelListener listener) {
        synchronized (streamSoundLevelListenersSyncRoot) {
            /*
             * Implement streamAudioLevelListeners as a copy-on-write storage so that iterators over
    ...
    public void setImage(byte[] image) {
        ...
        // Fire the Event
        fireCallPeerChangeEvent(CallPeerChangeEvent.CALL_PEER_IMAGE_CHANGE, oldImage, image, null, -1);
    }
