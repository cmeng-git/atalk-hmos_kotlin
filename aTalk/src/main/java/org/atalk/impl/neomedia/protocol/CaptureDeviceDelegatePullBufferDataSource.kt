...
/**
 * Represents a `PullBufferDataSource` which is also a `CaptureDevice` through
 * delegation to a specific `CaptureDevice` .
 *
 * @author Damian Minkov
 * @author Lubomir Marinov
 */
open class CaptureDeviceDelegatePullBufferDataSource
/**
 * Initializes a new `CaptureDeviceDelegatePullBufferDataSource` instance which
 * delegates to a specific `CaptureDevice` in order to implement its
 * `CaptureDevice` functionality.
 *
 * @param captureDevice
 * the `CaptureDevice` the new instance is to delegate to in order to provide its
 * `CaptureDevice` functionality
 */
(
        /**
         ...
         * `CaptureDevice` functionality.
         */
        protected val captureDevice: CaptureDevice?) : PullBufferDataSource(), CaptureDevice {
    /**
     * Implements [PullBufferDataSource.getStreams]. Delegates to the wrapped
     * `CaptureDevice` if it implements `PullBufferDataSource`; otherwise, returns an
     * empty array with `PullBufferStream` element type.
     *
     * @return an array of `PullBufferStream`s as returned by the wrapped
     * `CaptureDevice` if it implements `PullBufferDataSource`; otherwise, an
     * empty array with `PullBufferStream` element type
     */
    override fun getStreams(): Array<PullBufferStream> {
        return if (captureDevice is PullBufferDataSource) (captureDevice as PullBufferDataSource).streams else EMPTY_STREAMS
    }
     ...
     *
     * @throws IOException
     * if the wrapped `CaptureDevice` throws such an exception
     */
     ...
     * of `DataSource` if any; otherwise, returns
     * `DataSource#DURATION_UNKNOWN`
     */
    ...
    companion object {
         ...
         */
        protected val EMPTY_STREAMS = emptyList<PullBufferStream>().toTypedArray()
    }
}