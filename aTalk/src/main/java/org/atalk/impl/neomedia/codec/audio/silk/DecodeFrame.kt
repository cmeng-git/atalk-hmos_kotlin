...
object DecodeFrame {
     ...
     */
    fun SKP_Silk_decode_frame(psDec: SKP_Silk_decoder_state?,  /*
																	 * I/O Pointer to Silk decoder
																	 * state
																	 */
            pOut: ShortArray,  /* O Pointer to output speech frame */
            pOut_offset: Int, pN: ShortArray,  /* O Pointer to size of output frame */
            pCode: ByteArray,  /* I Pointer to payload */
            pCode_offset: Int, nBytes: Int,  /* I Payload length */
            action: Int,  /* I Action from Jitter Buffer */
            decBytes: IntArray /* O Used bytes to decode this frame */
    ): Int {
        var action = action
        val sDecCtrl = SKP_Silk_decoder_control()
        var L: Int
        val fs_Khz_old: Int
        val LPC_order_old: Int
        var ret = 0
        val Pulses = IntArray(Define.MAX_FRAME_LENGTH)
        ...
        if (action == 0) {
            ...
            if (psDec.sRC.error != 0) {
                ...
                ret = if (psDec.sRC.error == Define.RANGE_CODER_DEC_PAYLOAD_TOO_LONG) {
                    Errors.SKP_SILK_DEC_PAYLOAD_TOO_LARGE
                } else {
                    Errors.SKP_SILK_DEC_PAYLOAD_ERROR
                }
            } else {
        ...
        /* HP filter output */
        /** */
        Typedef.SKP_assert(psDec.fs_kHz == 12 && L % 3 == 0 || psDec.fs_kHz != 12 && L % 2 == 0)
        Biquad.SKP_Silk_biquad(pOut, pOut_offset, psDec.HP_B, psDec.HP_A, psDec.HPState, pOut,
                pOut_offset, L)
        /** */
