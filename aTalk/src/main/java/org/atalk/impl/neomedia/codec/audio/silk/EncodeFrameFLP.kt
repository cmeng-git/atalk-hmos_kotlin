...
object EncodeFrameFLP {
     ...
     * @return
     */
    fun SKP_Silk_encode_frame_FLP(psEnc: SKP_Silk_encoder_state_FLP?,  /* I/O Encoder state FLP */
            pCode: ByteArray?, pCode_offset: Int, pnBytesOut: ShortArray,  /* I/O Number of payload bytes; */
            /* input: max length; output: used */
            pIn: ShortArray?,  /* I Input speech frame */
            pIn_offset: Int): Int {
        val sEncCtrl = SKP_Silk_encoder_control_FLP()
        var k: Int
        val nBytes = IntArray(1)
        var ret = 0
        val x_frame: FloatArray?
        val res_pitch_frame: FloatArray
        val x_frame_offset: Int
        val res_pitch_frame_offset: Int
        val pIn_HP = ShortArray(Define.MAX_FRAME_LENGTH)
        val pIn_HP_LP = ShortArray(Define.MAX_FRAME_LENGTH)
        val xfw = FloatArray(Define.MAX_FRAME_LENGTH)
        val res_pitch = FloatArray(2 * Define.MAX_FRAME_LENGTH + Define.LA_PITCH_MAX)
        var LBRR_idx: Int
        var frame_terminator: Int

        /* Low bitrate redundancy parameters */
        val LBRRpayload = ByteArray(Define.MAX_ARITHM_BYTES)
        val nBytesLBRR = ShortArray(1)
        val FrameTermination_CDF: IntArray?
        sEncCtrl.sCmn.Seed = psEnc!!.sCmn.frameCounter++ and 3
        /** */
        ...
        k = 0
        while (k < 8) {
            x_frame[x_frame_offset + psEnc.sCmn.la_shape + k * (psEnc.sCmn.frame_length shr 3)] += (1 - (k and 2)) * 1e-6f
            k++
        }
        /* TEST*************************************************************************** */
        /**
        ...
        /** */
        nBytesLBRR[0] = Define.MAX_ARITHM_BYTES.toShort()
        ...
        if (psEnc.sCmn.nFramesInPayloadBuf * Define.FRAME_LENGTH_MS >= psEnc.sCmn.PacketSize_ms) {
            LBRR_idx = psEnc.sCmn.oldest_LBRR_idx + 1 and Define.LBRR_IDX_MASK
            ...
            if (pnBytesOut[0] >= nBytes[0]) {
                ...
                pnBytesOut[0] = nBytes[0].toShort()

                /* Update FEC buffer */
                System.arraycopy(LBRRpayload, 0,
                        psEnc.sCmn.LBRR_buffer[psEnc.sCmn.oldest_LBRR_idx]!!.payload, 0, nBytesLBRR[0].toInt())
                psEnc.sCmn.LBRR_buffer[psEnc.sCmn.oldest_LBRR_idx]!!.nBytes = nBytesLBRR[0].toInt()
                ...
                psEnc.sCmn.oldest_LBRR_idx = psEnc.sCmn.oldest_LBRR_idx + 1 and Define.LBRR_IDX_MASK
        ...
        psEnc.BufferedInChannel_ms -= Define.FRAME_LENGTH_MS.toFloat()
     ...
     * Input signal.
     */
    fun SKP_Silk_LBRR_encode_FLP(psEnc: SKP_Silk_encoder_state_FLP?,  /* I/O Encoder state FLP */
            psEncCtrl: SKP_Silk_encoder_control_FLP,  /* I/O Encoder control FLP */
            pCode: ByteArray?,  /* O Payload */
            pnBytesOut: ShortArray,  /* I/O Payload bytes; in: max; out: used */
            xfw: FloatArray? /* I Input signal */
    ) {
        val Gains_Q16 = IntArray(Define.NB_SUBFR)
        var k: Int
        val TempGainsIndices = IntArray(Define.NB_SUBFR)
        val frame_terminator: Int
        val nBytes = IntArray(1)
        val nFramesInPayloadBuf: Int
        val TempGains = FloatArray(Define.NB_SUBFR)
        val typeOffset: Int
        val LTP_scaleIndex: Int
        var Rate_only_parameters = 0
        ...
        if (psEnc!!.sCmn.LBRR_enabled != 0) {
            ...
            if (psEnc.sCmn.Complexity > 0 && psEnc.sCmn.TargetRate_bps > Rate_only_parameters) {
                if (psEnc.sCmn.nFramesInPayloadBuf == 0) {
                    ...
                    psEnc.sNSQ_LBRR = (psEnc.sNSQ.clone() as SKP_Silk_nsq_state)
                ...
                val LBRRprevLastGainIndex_ptr = IntArray(1)
                ...
                Arrays.fill(psEnc.sCmn.q_LBRR, 0.toByte())
            ...
            /* Encode Parameters */
            /** */
            nFramesInPayloadBuf = if (psEnc.sCmn.sRC_LBRR.error != 0) {
                /* Encoder returned error: Clear payload buffer */
                0
            } else {
