 ...
 */
class VP8Encoder : AbstractCodec2("VP8 Encoder", VideoFormat::class.java, SUPPORTED_OUTPUT_FORMATS) {
    /**
    ...
    /**
     * Initializes a new `VP8Encoder` instance.
     */
    init {
        arrayOf<VideoFormat>(YUVFormat(
                null,  /* size */
    ...
    override fun doOpen() {
        ...
        if (ret != VPX.CODEC_OK)
            throw RuntimeException("Failed to initialize encoder, libvpx error:\n" + codec_err_to_string(ret))
     ...
     * {@inheritDoc}
     *
     * Encode the frame in `inputBuffer` (in `YUVFormat`) into a VP8 frame (in `outputBuffer`)
     *
    ...
    override fun doProcess(inBuf: Buffer, outBuf: Buffer): Int {
        ...
        if (!leftoverPackets) {
            val format: YUVFormat = inBuf.format as YUVFormat
            ...
            if (result != VPX.CODEC_OK) {
                Timber.w("Failed to encode a frame: %s", codec_err_to_string(result))
        ...
        if (pkt != 0L && VPX.codec_cx_pkt_get_kind(pkt) == VPX.CODEC_CX_FRAME_PKT) {
            val size = VPX.codec_cx_pkt_get_size(pkt)
            val data = VPX.codec_cx_pkt_get_data(pkt)
        ...
        return if (leftoverPackets) ret or PlugIn.INPUT_BUFFER_NOT_CONSUMED
        else {
            ret
        }
    }
    ...
    private fun updateSize(w: Int, h: Int) {
        ...
        mHeight = h
        img = VPX.img_alloc(img, VPX.IMG_FMT_I420, mWidth, mHeight, 1)
        if (img == 0L) throw RuntimeException("Failed to re-initialize VP8 encoder")
        if (cfg != 0L) {
        ...
        if (vpctx != 0L) {
            ...
            val ret: Int = VPX.codec_enc_init(vpctx, INTERFACE, cfg, flags)
            if (ret != VPX.CODEC_OK)
                throw RuntimeException("StreamManagement. Failed to re -initialize VP8 encoder, libvpx error :\n"
                        + codec_err_to_string(ret))
        }
    }
    ...
    override fun setInputFormat(format: Format): Format? {
        ...
        if (yuvFormat.offsetU > yuvFormat.getOffsetV()) return null
    ...
    companion object {
         ...
         */
        private const val INTERFACE: Int = VPX.INTERFACE_VP8_ENC

        /**
         * Default output formats
         */
        private val SUPPORTED_OUTPUT_FORMATS: Array<VideoFormat> = arrayOf(VideoFormat(Constants.VP8))
    }
}