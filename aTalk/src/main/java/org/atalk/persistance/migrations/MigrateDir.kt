...
object MigrateDir {
     ...
     */
    fun purgeAvatarCache() {
        val avatarCacheDir = File(aTalkApp.globalContext.filesDir, "/avatarcache")
        if (avatarCacheDir.exists()) {
            try {
                FileBackend.deleteRecursive(avatarCacheDir)
            } catch (e: IOException) {
                Timber.e("Failed to purge aTalk avatarCache directory: %s", e.message)
            }
        }
    } //    /**
